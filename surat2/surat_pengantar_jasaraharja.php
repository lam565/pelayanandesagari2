...
$pdf->Cell(15,2, '                                             ____________________ ', '0', 1, 'C');
$pdf->Ln(-0.5);
$pdf->SetFont('Arial','','12');
$pdf->Line(2,4.5,20,4.5);
$pdf->SetLineWidth(0);

$query = "SELECT *FROM biodata_wni,data_keluarga,jasaraharja,staf,jabatan,
...
$pdf->Cell(9,0, '      Gunungkidul, menerangkan dengan sesungguhnya  bahwa : ', '0', 1, 'L'); 
$pdf->Ln(0.8);
...
$pdf->Ln(0.5); 
$pdf->Cell(5,0, '                    Alamat	                             :	'.$row['ALAMAT']. ', Wonosari, Gunungkidul', '0', 1, 'L');
$pdf->Ln(0.5);
$pdf->Cell(5,0, '             	     Pekerjaan	                           :	'.$row['pekerjaan']. ' ', '0', 1, 'L');
$pdf->Ln(0.5); 
$pdf->Cell(5,0, '              	    Status Perkawinan	             :	'.$row['status_perkawinan']. ' ', '0', 1, 'L');  
$pdf->Ln(0.5); 
$pdf->Cell(5,0, '             	     Agama	                               :	'.$row['nama_agama']. ' ', '0', 1, 'L');
$pdf->Ln(0.8); 

$pdf->Cell(5,0, '              Menerangkan bahwa data orang tersebut yang diatas benar-benar penduduk Padukuhan  ', '0', 1, 'L');
$pdf->Ln(0.5);
$pdf->Cell(5,0, '      '.$row['ALAMAT']. ', Kalurahan Gari ,Kecamatan Wonosari, Kabupaten Gunungkidul', '0', 1, 'L');
$pdf->Ln(0.8);
$pdf->Cell(5,0, '              Orang tersebut diatas pada  hari '.$row['tanggal_kejadian']. ',  mengalami Kecelakaan di kota  '.$row['tempat_kejadian'].  ' ', '0', 1, 'L');
$pdf->Ln(0.5);
$pdf->Cell(5,0, '       Kecelakaan tersebut mengakibatkan  berat dan harus periksa ke dokter di Rumah Sakit    ', '0', 1, 'J');
$pdf->Ln(0.8);
$pdf->Cell(5,0, '              Surat keterangan ini di berikan untuk mencari: Mencari rekomendasi  jaminan Jasa Raharja    ', '0', 1, 'J'); 
$pdf->Ln(0.8);
$pdf->Cell(5,0, '             Demikian   Surat   Keterangan   ini  dibuat   agar   dapat   dipergunakan   sebagaimana     ', '0', 1, 'J');
$pdf->Ln(0.5);
$pdf->Cell(5,0, '     mestinya.     ', '0', 1, 'J');
$pdf->Ln(1.5);
